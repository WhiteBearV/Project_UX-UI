...
class Config:
    ...
    MODEL_BOOK_PATH = "yolo11n.pt" # หรือ best.pt ตามที่คุณมี
    PERSON_CLASS_ID = 0
...
class RobotArm:
    ...
    def start_scan(self):
        self.set_light("RED")
        return self.send_command(f"SCAN S1 {Config.SCAN_MIN} {Config.SCAN_MAX} {Config.SCAN_SPEED}", wait=True)
    ...
    try:
        while True:
            ...
            if state == "INIT":
                ...
                # 1. เงื่อนไขหนังสือ: เจอแล้วไปเลยทันที (ไม่ต้องรอ 2 วิ)
                if cached_book:
                    robot.book_mode_on()
                    state = "BOOK"
                ...
                if cached_book:
                    ...
                    last_seen_time = now # รีเซ็ตเวลาป้องกันหลับทันทีหลังตื่น
                    state = "SCAN"
                    scan_active = False
