...
class Config:
    ...
    SCAN_MIN = 15
    SCAN_MAX = 165
    SCAN_SPEED = 20   # UNO clamp สูงสุด 180
...
class RobotArm:
    ...
    # ✅ Buzzer (ถ้าเรียกจากภายนอก)
    def beep(self, duration_ms=120, freq=2000, wait_ok=False):
        # ปกติแนะนำ wait_ok=False กันหน่วง
        return self.cmd(f"BEEP {int(duration_ms)} {int(freq)}", wait_ok=wait_ok)[0]
...
def main():
    ...
    book_end = None
    book_last_sec = None   # ✅ กันบี๊บซ้ำในวินาทีเดียว (5..1)
    hold_end = None
    rest_end = None
    sleep_end = None
        ...
        while True:
            ...
            if state == "WAIT_ARM":
                ...
                if not armed:
                    ...
                    # ✅ 5 วินาทีสุดท้ายก่อนจบ BOOK mode: ดังทุกวินาที (5,4,3,2,1)
                    if book_end is not None:
                        remaining = book_end - now

                        if 0 < remaining <= 5.0:
                            tick = int(remaining) + 1  # 5..1
                            if book_last_sec != tick:
                                # ✅ ส่งแบบไม่รอ OK กันหน่วง
                                robot.cmd("BEEP 120 2000", wait_ok=False)
                                book_last_sec = tick
